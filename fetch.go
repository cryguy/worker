...
// setupFetchWithConfig registers Go-backed fetch helpers and evaluates
// the JS polyfill.
func setupFetchWithConfig(vm *quickjs.VM, cfg EngineConfig, el *eventLoop) error {
	...
	registerGoFunc(vm, "__fetchStart", func(reqIDStr, argsJSON string) (string, error) {
		...
		go func() {
			...
			if httpErr != nil {
				...
				if capturedRedirectMode == "error" {
					resultCh <- fetchResult{err: fmt.Errorf("fetch failed: redirect mode is 'error'")}
					return
				}
				if abortedBySignal {
					resultCh <- fetchResult{err: fmt.Errorf("The operation was aborted.")}
					return
				}
				resultCh <- fetchResult{err: fmt.Errorf("fetch: %s", httpErr.Error())}
				return
			}
			...
			if readErr != nil {
				resultCh <- fetchResult{err: fmt.Errorf("fetch: reading body: %s", readErr.Error())}
				return
			}
		...
		el.addPendingFetch(&pendingFetch{resultCh: resultCh, fetchID: fetchID})
		return fetchID, nil
	}, false)

	// __fetchAbort(reqID, fetchID)
	registerGoFunc(vm, "__fetchAbort", func(reqIDStr, fetchID string) {
		reqID := parseReqID(reqIDStr)
		callFetchCancel(reqID, fetchID)
	}, false)

	return evalDiscard(vm, fetchJS)
}
